...
def obtener_expedientes_filtrados(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Convertir fechas string a datetime objects con zona horaria
            from django.utils import timezone
            fecha_inicio_dt = datetime.strptime(fecha_inicio, '%Y-%m-%d')
            fecha_fin_dt = datetime.strptime(fecha_fin, '%Y-%m-%d')
            
            # Ajustar fechas para incluir todo el día
            # Usar fechas naive para comparar con fecha_creacion que ya está en UTC
            fecha_inicio_ajustada = fecha_inicio_dt.replace(hour=0, minute=0, second=0, microsecond=0)
            fecha_fin_ajustada = fecha_fin_dt.replace(hour=23, minute=59, second=59, microsecond=999999)
            
            # Log para depuración
            logger.info(f'Filtros de fecha - Inicio: {fecha_inicio} -> {fecha_inicio_dt}')
            logger.info(f'Filtros de fecha - Fin: {fecha_fin} -> {fecha_fin_dt}')
            logger.info(f'Zona horaria del sistema: {timezone.get_current_timezone()}')
            logger.info(f'Consulta SQL generada: fecha_creacion__date__gte={fecha_inicio_dt} AND fecha_creacion__date__lte={fecha_fin_dt}')
            
            # Usar __date para comparar solo la fecha, ignorando la hora
            filtros = Q(fecha_creacion__date__gte=fecha_inicio_dt) & Q(fecha_creacion__date__lte=fecha_fin_dt)
            ...
            for expediente in expedientes:
                expedientes_data.append({
                    ...
                    'fecha_creacion': expediente.fecha_creacion.strftime('%Y-%m-%d'),
                    'departamento': expediente.departamento.nombre if expediente.departamento else 'Sin departamento',
                    'solicitante': expediente.creado_por.get_full_name() if expediente.creado_por else 'Usuario no especificado'
                })
