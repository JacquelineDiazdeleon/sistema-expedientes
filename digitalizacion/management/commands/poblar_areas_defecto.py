...
class Command(BaseCommand):
    help = 'Pobla las áreas por defecto para todos los tipos de expediente'

    def handle(self, *args, **options):
        self.stdout.write('Poblando áreas por defecto...')
        
        # Definir las 17 áreas por defecto (anteriormente etapas)
        areas_defecto = [
            {
                'nombre': 'inicio',
                'titulo': 'INICIO',
        ...
        # Obtener el usuario sistema (o crear uno temporal)
        ...
        for tipo in tipos_expediente:
            ...
            # Crear áreas para cada subtipo
            for subtipo_key, subtipo_display in subtipos:
                self.stdout.write(f'  Procesando subtipo: {subtipo_display}')
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f'  - Total áreas por tipo: {len(areas_defecto)}'
            )
        )
        
        # Mostrar resumen por tipo
        self.stdout.write('\nResumen por tipo de expediente:')
        for tipo in tipos_expediente:
            count = AreaTipoExpediente.objects.filter(tipo_expediente=tipo).count()
            tipo_display = dict(Expediente.TIPO_CHOICES)[tipo]
            self.stdout.write(f'  - {tipo_display}: {count} áreas')
            
            # Mostrar subtipos
            subtipos = AreaTipoExpediente.get_subtipos_por_tipo(tipo)
            for subtipo_key, subtipo_display in subtipos:
                subtipo_count = AreaTipoExpediente.objects.filter(
                    tipo_expediente=tipo, 
                    subtipo_expediente=subtipo_key
                ).count()
                self.stdout.write(f'    - {subtipo_display}: {subtipo_count} áreas')
