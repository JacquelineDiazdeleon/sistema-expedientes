...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Mostrar resumen por tipo de expediente
        for tipo, subtipos in areas_por_tipo.items():
            tipo_exp, subtipo = tipo
            subtipo_str = f" ({subtipo})" if subtipo else ""
            self.stdout.write(f"\n{tipo_exp}{subtipo_str}:")
            for area in areas_por_tipo[tipo]:
                self.stdout.write(f"  - {titulos_areas.get(area, area)}")
        
        self.stdout.write(self.style.SUCCESS('\n¡Proceso de actualización de áreas completado!'))
