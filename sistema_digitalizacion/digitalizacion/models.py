...
class AreaTipoExpediente(models.Model):
    """Modelo para las áreas/secciones de documentos según el tipo de expediente"""
    tipo_expediente = models.CharField(max_length=20, choices=Expediente.TIPO_CHOICES)
    etapa = models.CharField(max_length=30, choices=Expediente.ESTADO_CHOICES)
    titulo = models.CharField(max_length=200)
    ...
    class Meta:
        ...
        verbose_name_plural = "Áreas de Tipos de Expedientes"
        ordering = ['tipo_expediente', 'orden']
        unique_together = ['tipo_expediente', 'slug']

    def __str__(self):
        return f"{self.get_tipo_expediente_display()} - {self.titulo}"
